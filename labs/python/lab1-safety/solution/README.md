...
```bash
python main.py another-agent.yaml
```
...
This lab expects its configuration from `../../.env` (the shared Python lab config file at `labs/python/.env`).

Authentication uses:

- Service principal auth when `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, and `AZURE_CLIENT_SECRET` are present
