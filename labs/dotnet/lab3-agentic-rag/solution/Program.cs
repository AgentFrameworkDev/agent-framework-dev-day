...
class Program
{
    ...
    static Workflow BuildWorkflow(Dictionary<string, AIAgent> agents)
    {
        ...
        var differenceExecutor = new SpecialistExecutor("Difference", agents["difference"]);
        var intersectionExecutor = new SpecialistExecutor("Intersection", agents["intersection"]);
        var multiHopExecutor = new SpecialistExecutor("MultiHop", agents["multi_hop"]);
        var comparativeExecutor = new SpecialistExecutor("Comparative", agents["comparative"]);
        ...
        builder.AddSwitch(classifierExecutor, sb => sb
            ...
            .AddCase(CategoryConditions.Is("difference"), differenceExecutor)
            .AddCase(CategoryConditions.Is("intersection"), intersectionExecutor)
            .AddCase(CategoryConditions.Is("multi_hop"), multiHopExecutor)
            .AddCase(CategoryConditions.Is("comparative"), comparativeExecutor)
            .WithDefault(semanticSearchExecutor)
        )
        .WithOutputFrom(yesNoExecutor, countExecutor, differenceExecutor,
            intersectionExecutor, multiHopExecutor, comparativeExecutor, semanticSearchExecutor);
