...
async def main():
    ...
    if test_questions:
        ...
        #events = await drain_events(workflow.run(first_question, stream=True))
        #pending_requests = handle_workflow_events(events, verbose=False)
        
        workflow_result = workflow.run(first_question, stream=True)
        pending_requests = handle_workflow_events([event async for event in workflow_result])
        ...
        for i, question in enumerate(remaining_questions, 2):
            if pending_requests:
                ...
                #events = await drain_events(workflow.run(responses=responses, stream=True))
                #pending_requests = handle_workflow_events(events, verbose=False)
                workflow_result = workflow.run(first_question, stream=True)
                pending_requests = handle_workflow_events([event async for event in workflow_result])
            else:
...
async def interactive_mode():
    ...
    while True:
        try:
            ...
            print()
            events = await drain_events(workflow.run(user_input, stream=True))
